...
async function fetchTitleFromReply(event) {
  ...
  const response = await axios.get(`https://audio-recon-ahcw.onrender.com/kshitiz?url=${encodeURIComponent(attachment.url)}`);
...
async function downloadAudioFromYouTube(videoId, title) {
  ...
  const bestStream = Object.values(streams).filter(v => v?.download_url).sort((a, b) => (b.bitrate || 0) - (a.bitrate || 0))[0];
...
async function handleSingCommand({ api, event, args }) {
  try {
    ...
    if (!query) return api.sendMessage("⚠️ | Provide a search term or reply to audio/video.", event.threadID, event.messageID);
    ...
    // Send audio
    api.sendMessage({ body: `🎵 ${audioTitle}`, attachment: fs.createReadStream(tempAudioPath) }, event.threadID, () => {
